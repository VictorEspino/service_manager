<div>
    <x-slot name="header">
        {{ __('Listas de Valores') }}
    </x-slot>
    <div class="w-full flex flex-col px-5 md:py-6">
        <div class="w-full">
            <x-jet-section-title>
                <x-slot name="title">Administracion de listas de valores</x-slot>
                <x-slot name="description">Permite visualizar y dar mantenimiento a las listas de valores que seran usadas para campos personalizados de topicos y tickets</x-slot>
            </x-jet-section-title>
        </div>
    </div>
    ...
    <div class="w-full flex flex-col space-y-3 py-5 px-5">
        ...
        @foreach ($listas as $lista)
            ...
            <div class="w-full flex flex-row bg-white rounded-lg shadow-lg p-3 border border-blue-200">
                <div class="w-1/3 text-gray-700 font-semibold text-xl px-3">{{$lista->nombre}}</div>
                <div class="w-1/2 text-gray-700 font-semibold text-xs px-3">{{$lista->descripcion}}</div>
                ...
                <div class="w-1/12">
                    @livewire('lista.update-lista',['id_lista'=>$lista->id,key($lista->id)])
                </div>
            </div>
        @endforeach
        @if($registros==0)
